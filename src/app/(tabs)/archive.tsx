...
export default function DarkInspirationArchive() {
    ...
    const completedGoals = activeCategory === 'All'
        ? allCompleted
            ...
            <ScrollView className="flex-1 z-10 pt-4" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 24 }}>
                        {CATEGORIES.map((cat) => {
                            ...
                            return (
                                ...
                                >
                                    {cat === 'Travel' && <MaterialIcons name="flight" size={14} color={active ? 'white' : '#9ca3af'} />}
                                    {cat === 'Adventures' && <MaterialIcons name="hiking" size={14} color={active ? 'white' : '#9ca3af'} />}
                ...
                <View className="px-6 pb-4">
                    <View className="flex-row items-center justify-between mb-5">
                        ...
                        <Text className="text-xs text-gray-500">{completedGoals.length} {completedGoals.length === 1 ? 'item' : 'items'}</Text>
                    </View>

                    {completedGoals.length === 0 ? (
                        <View className="items-center py-16">
                            <MaterialIcons name="photo-album" size={48} color="#374151" />
                        ...
                        >
                            {completedGoals.map((goal: Goal, index: number) => {
                                ...
                                const iconCol = ['#f87171', '#2dd4bf', '#60a5fa', '#a78bfa'][index % 4];
                                const catIcon = goal.category === 'Foodie' ? 'restaurant' : goal.category === 'Stays' ? 'hotel' : goal.category === 'Travel' ? 'flight' : goal.category === 'Milestone' ? 'star' : 'hiking';
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <LinearGradient colors={gradient as [string, string]} start={{ x: 1, y: 0 }} end={{ x: 0, y: 1 }} className="absolute inset-0 z-0" />
                                        ...
                                        <View className="relative z-20 h-full flex-col justify-between">
                                            <View className="w-11 h-11 rounded-full bg-black/40 border border-white/10 items-center justify-center">
                                                <MaterialIcons name={catIcon as any} size={22} color={iconCol} />
                                            </View>
                                            <View>
                                                <Text className="text-2xl font-bold text-white leading-tight mb-1" numberOfLines={2}>{goal.title}</Text>
                                                <View className="flex-row items-center mt-1">
                                                    <MaterialIcons name="place" size={12} color="#9ca3af" />
                                                    <Text className="text-gray-400 text-xs ml-1" numberOfLines={1}>
                                                        {goal.location.city || goal.location.country || 'No location'}
                                                    </Text>
                                                </View>
                                                {goal.completedAt && (
